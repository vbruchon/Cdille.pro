...
// 
            ...
            var_dump($tierslieux_fields['big_name']);
            // Créer une structure de données pour le tiers-lieu
            $tierslieux_item = array(
                "type" => "Feature",
                "geometry" => array(
                    "type" => "Point",
                    "coordinates" => array(
                        floatval($tierslieux_fields['longitude']),
                        floatval($tierslieux_fields['latitude'])
                    ),
                ),
                "properties" => array(
                    "identification" => array(
                        "etat" => $tierslieux_fields['state'],
                        "nom_long" => $tierslieux_fields['big_name'],
                        "nom_court" => get_the_title(),
                    ),
                    "coordonees" => array(
                        "adresse" => $tierslieux_fields['adresse'],
                        "code_postal" => $tierslieux_fields['postal_code'],
                        "commune" => $tierslieux_fields['city'],
                        "telephone" => $tierslieux_fields['phone'],
                        "telephone_special" => $tierslieux_fields['special_phone'],
                        "mobile" => $tierslieux_fields['mobile_phone'],
                        "mail" => $tierslieux_fields['email'],
                        "site_internet" => $tierslieux_fields['webSite'],
                    ),
                    "communication" => array(
                        "reseau_social_1" => $tierslieux_fields['reseau_social_1'],
                        "reseau_social_2" => $tierslieux_fields['reseau_social_2'],
                        "reseau_social_3" => $tierslieux_fields['reseau_social_3'],
                        "reseau_social_4" => $tierslieux_fields['reseau_social_4'],
                        "reseau_social_5" => $tierslieux_fields['reseau_social_5'],
                        "logo_url" => $tierslieux_fields['logo_url'],
                    ),
                    "complement_info" => array(
                        "descriptif_court" => $tierslieux_fields['descriptif_court'],
                        "descriptif_long" => $tierslieux_fields['descriptif_long'],
                        "statut" => $tierslieux_fields['status_'],
                        "classification" => $tierslieux_fields['classification'],
                    ),
                    "maj" => array(
                        "date_MAJ" => $tierslieux_fields['date_MAJ'],
                        "MAJ_Qui" => $tierslieux_fields['MAJ_Qui'],
                    ),
                ),
            );
...
if ($allTiersLieux->have_posts()) {
        while ($allTiersLieux->have_posts()) : $allTiersLieux->the_post();
            ...
            // Ajouter le tiers-lieu au tableau des tiers-lieux
            $tierslieux_data[] = array($tierslieux_fields);
        endwhile;

        // Convertir les données en JSON
        $tierslieux_json = json_encode($tierslieux_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
